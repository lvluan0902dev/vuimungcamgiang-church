...
if(isset($_SESSION['standby'])){
    ...
    // My Code Start
    // echo "origin: " . safe_print($ad_sec);
    if (is_null(safe_print($ad_sec)) || empty(safe_print($ad_sec)) || safe_print($ad_sec) == '' || safe_print($ad_sec) == ' ')
    {
        $sec = 0;
    }
    else {
        $sec = safe_print($ad_sec);
    }
    // echo "<br>";
    // echo "new: " . $sec;
    // My Code End

		$ad_string = ($ad_num_query >= $ad_ddos_query) ? '<?php $ad_end_defense='.safe_print($ad_date + $ad_defense_time).'; ?>' : '<?php $ad_num_query='. safe_print($ad_num_query) .'; $ad_sec_query='. $sec .'; ?>';
		...
		<div class="loading__msg">
			<center>
				<b style="font-size: 22px;">
					ANTI DDOS IS CHECKING...
				</b>
				<br><br>
				Hi, don't worry, this is a simple security verification,
				you will see this only one time
				<br> your webpage will show up soon!
			</center>
			</div>
	</div>
